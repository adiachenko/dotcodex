...
Use this reference for the concrete Chrome steps that support the main `SKILL.md`.

Assume ChatGPT Pro starts with no project context until the prompt supplies it. Login and conversation state carry over through the browser session. Repository context must come from the prompt.

## Setup

- Read and follow the installed Chrome control instructions before browser work.
- Connect to the user's existing Chrome session and name the browser task as those instructions require.
...
- If Chrome control is unavailable or cannot reconnect, stop and report that the consultation is unavailable. Do not switch to another browser-control tool.
...
Reuse a previous Pro conversation only for a user-requested follow-up. Continue that thread or start a new one only when the user explicitly asks for another Pro turn. If a follow-up would help, suggest it and wait for approval before sending it.
...
Select the visible `Pro` label and let ChatGPT map it to the current underlying model. Do not hardcode a numbered model name. If no Pro option is visible, stop and report that the requested Pro call is unavailable.
...
3. Save it to a unique temp file with restrictive permissions, keep its printed absolute path, and retain the file until the turn is fully sent and the answer is collected.
...
For a large prompt, fill the composer with the complete file contents instead of typing it character by character.
...
Delete the captured temp file after collecting the answer or abandoning the run. Do not rely on a shell exit trap across tool calls or reuse a stale prompt file.
...
Prefer just enough context. A short briefing and a few relevant excerpts beat large slices of the repo.
...
When continuing an existing conversation, include a short recap of what changed or what new decision is needed so the follow-up does not depend on the model inferring every change.
...
- If it is unclear whether the message was submitted, inspect the page again before pressing Enter or clicking the submit button.
...
- Keep the same tab open and check progress infrequently within the active Codex turn.
- Visible generation, research, or tool activity means the response is still running, even if the answer text has stopped changing. If completion is ambiguous, keep waiting; a delegated progress update or wait timeout is not completion.
...
- Keep the useful reasoning, tradeoffs, risks, and recommended next step.
...
- If Chrome control disconnects, reconnect and return to the recorded conversation URL before starting a new chat or submitting again.
...
- If one way of filling the composer fails, reuse the saved prompt file and try another way to fill the entire prompt. Do not reconstruct the prompt by hand or send a partial version.
