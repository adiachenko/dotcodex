---
name: design-tests
description: Use when planning, creating, editing, fixing, organizing, or reviewing tests or test coverage, including interpreting mutation-testing results, choosing test boundaries, scenarios, test data, mocks, or assertions, and removing redundant or implementation-mirroring checks. Use alongside project- or framework-specific testing skills.
---

# Design Tests

Write tests as executable examples for maintainers. Make each test a readable, coherent proof of one supported behavior, not a spell checker for the production code.

One coherent proof does not mean one assertion or an artificially small test. Use every observation needed to establish one invariant, transition, or outcome. Split rules that can change independently or need different failure explanations.
...
- Prioritize by the likelihood and consequence of regression, not by how easy a case is to generate or assert.
...
- Do not manufacture unsupported internal states merely to exercise defensive branches. Test invalid input where it crosses a supported boundary and the response is part of the contract.

## Work from the outside in

- Choose the test level by the behavior's owner, not by how much framework infrastructure the test boots. Prefer a supported public boundary when it exposes the invariant; otherwise test the owning component. Assert observable outcomes rather than how internal objects interact.
- Give each guarantee one primary proof across the suite. Public-boundary and lower-level proofs are redundant only when the same plausible wrong implementation would make both fail.
...
- Allow lower-level tests written while implementing to be temporary. Before finalizing, remove or consolidate those made redundant by a clearer public proof.
...
- Pin a representation exactly only in the test that owns that representation as a contract. Make other tests assert its meaning instead of repeating its complete shape.

## Build a discriminating scenario

- Use the smallest deterministic scene that makes plausible wrong implementations visibly fail.
- Build rejection and validation cases from a known-valid scenario, changing only the facts needed to cross the rejected boundary. Prove that the intended rule, not an unrelated invalid precondition, caused the failure.
...
- Keep every decisive identity, relationship, state, value, and time visible at the test call site. Let fixtures, factories, and helpers hide background mechanics, but not the condition that selects the behavior or the outcome being proved.
...
## Assert the smallest complete semantic outcome

Assert the smallest complete outcome that distinguishes correct behavior from a plausible wrong result. Prefer meaningful domain values, identities, order, state changes, and effects over counts, presence checks, or field inventories. Keep expected results independent of the implementation under test. When rejection, filtering, idempotency, isolation, or a no-op is the behavior, prove the relevant state or effect did not change.

## Remove spell-checker coverage

Review the affected tests as a set, including existing public-boundary tests. Entering through a public boundary does not make every assertion or scenario valuable.
...
- Remove repeated proof wherever it appears. In adjacent public-boundary scenarios and lower layers, delete an overlapping assertion, scenario, or test when a clearer public proof already owns the guarantee and the overlap is not needed to complete a different named outcome.
...
## Review the proof, not just the result

Before finalizing, verify that each test's name, decisive setup, action, and outcome describe the same rule; that a plausible wrong implementation cannot pass because of a hidden default, unrelated failure, weak count, or assertion of the wrong record; and that every remaining assertion and scenario adds evidence to the named outcome instead of repeating proof already owned elsewhere.
